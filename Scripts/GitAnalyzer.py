...
def analyze_repository(path, commits_file_path = 'Commits.csv', file_commits_file_path = 'FileCommits.csv'):
    ...
    # Write the FileCommits.csv file
    print('Fetching file commits. This will take a very long time...')
    commits = build_file_commits(repo)
    print('Saving Commits')
    df_commits = pd.DataFrame(commits)
    df_commits.to_csv(file_commits_file_path)
